...
class NetworkService {
    ...
    private func fetchData5(latitude: CLLocationDegrees, longitude: CLLocationDegrees, completion: @escaping (Data?, Error?) -> ()) {
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/forecast?lat=\(latitude)&lon=\(longitude)&appid=\(appID)&units=metric") else {
            let error = NSError(domain: NSURLErrorDomain, code: NSURLErrorBadURL)
            completion(nil, error)
            return
        }
...
extension NetworkService: NetworkServiceProtocol {
    ...
    func getSeveralDaysWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees, completion: @escaping (SeveralDaysWeather?, Error?) -> ()) {
        fetchData5(latitude: latitude, longitude: longitude) { data, error in
            if let error = error {
                completion(nil, error)
                return
            }
